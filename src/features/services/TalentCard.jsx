...
const TalentCard = ({ profile }) => {
  ...
  // 🛠️ DATA NORMALIZATION to handle different backend structures
  ...
  const displaySkills = profile.topSkills || profile.skills || [];
  ...
  return (
    ...
    >
      ...
      <div className="mb-6">
        ...
        `}>
          {profile.about || profile.description || "Top-tier AI expert ready to build your next breakthrough product."}
        </p>
      ...
      <div className={`pt-4 mt-auto border-t flex items-center justify-between ${isLight ? "border-gray-100" : "border-[#2a2a2a]"}`}>
        ...
        <div className="flex items-center gap-2">
          <span className="relative flex h-2.5 w-2.5">
            <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${profile.status === 'Available' ? 'bg-green-400' : 'bg-red-400'}`}></span>
            <span className={`relative inline-flex rounded-full h-2.5 w-2.5 ${profile.status === 'Available' ? 'bg-green-500' : 'bg-red-500'}`}></span>
          </span>
