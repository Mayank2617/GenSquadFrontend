...
const ProfileDetails = ({ profile }) => {
  ...
  // Helper styles
  const sectionClass = `p-8 rounded-[24px] mb-6 border ${isLight ? "bg-white border-gray-100 shadow-sm" : "bg-[#121212] border-white/10 shadow-lg"
    }`;

  const headingClass = `text-xl font-space font-bold mb-6 ${isLight ? "text-gray-900" : "text-white"
    }`;

  // Calculate dynamic gradient
  let cumulative = 0;
  const gradientParts = profile.pieData.map((item) => {
    const start = cumulative;
    cumulative += item.usage;
    return `${item.color} ${start}% ${cumulative}%`;
  });

  // If data is empty or total < 100, we might want a fallback or background, 
  // but let's just render what we have. 
  // If no data, gray circle? 
  const gradientString = gradientParts.length > 0
    ? `conic-gradient(${gradientParts.join(", ")})`
    : `conic-gradient(#e5e7eb 0% 100%)`; // Default gray if empty
